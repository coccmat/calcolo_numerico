...
\section{Rappresentazione floating point}
\label{sec:rapresentazione_floating_point}
...
Inoltre si ha che l'errore relativo ottenuto è sempre minore o uguale alla \textbf{precisione di macchina} rappresentata come $k\beta^{t-1}$ dove 
\begin{itemize}
\item $k=1$ nel caso si utilizzi logiche di approsimazione tramite toroncamento.
\item $k=\frac{1}{2}$ nel caso si usi arrotondamento.
\end{itemize}
questo risultato è chiamato \textbf{teorema dell'errore di rappresentazione dei numeri reali}.\\
Dato che l'insieme dei floating point $F$ non è chiuso alle operazioni aritmetiche si possono verificare, oltre ai casi di \textbf{overflow} e \textbf{underflow}, dei casi in cui il numero non è rappresentabile pur essendo compreso tra il numero massimo e il numero minimo rapresentabile del insieme, difatti si ha che un operazione tra numeri floating point restituisce un numero reale che andrà trasformato seguendo la logica vista nel capitolo \nameref{sec:rapresentazione_floating_point}
